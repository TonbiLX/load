...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QVBoxLayout, QWidget,
                              QTableWidget, QPushButton, QComboBox, QLabel,
                              QHBoxLayout, QAbstractItemView, QProgressBar)
...
class ContainerLoadingApp(QMainWindow):
    ...
    def initUI(self):
        ...
        zoom_in_path = os.path.join(base_dir, "zoom_in.png")
        zoom_out_path = os.path.join(base_dir, "zoom_out.png")

        self.zoom_in_button = QPushButton()
        self.zoom_in_button.setIcon(QIcon(zoom_in_path))
        self.zoom_in_button.setIconSize(QPixmap(zoom_in_path).scaled(20, 20).size())
        self.zoom_in_button.setFixedSize(40, 40)
        self.zoom_in_button.clicked.connect(self.zoom_in)

        self.zoom_out_button = QPushButton()
        self.zoom_out_button.setIcon(QIcon(zoom_out_path))
        self.zoom_out_button.setIconSize(QPixmap(zoom_out_path).scaled(20, 20).size())
    ...
    def start_loading(self):
        ...
        vehicle_name = self.vehicle_selector.currentText()
        container_dims = self.vehicle_options[vehicle_name]

        self.load_thread = LoadThread(self, data, vehicle_name, container_dims)
